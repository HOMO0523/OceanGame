...
| 潜水、钓鱼、上岛 | 钓鱼/上岛/完整潜水列入事件系统扩展；当前只做 X 下潜/浮上、摄像机往返和海底移动的最小切片。 |
...
当前不要直接做完整钓鱼、完整潜水、上岛交互，而是先做“事件点推进器”；X 潜水只作为最小移动/镜头切片存在，不承诺氧气、奖励和海底采集：
...
| 岛屿/潜水/钓鱼 | 当前潜水仅有 X 下潜/浮上镜头切片，其余事件按钮仍作为灰色/待做入口。 | 后续场景/小游戏。 |
...
| 生存 | 有体力、水分、饱食度 tick，背包消耗品可恢复数值。 | 危险/死亡反馈、事件驱动调参仍需打磨。 |
| 背包 | 有堆叠资源、物品槽、玩家可见背包 UI、拖拽/合并/交换和右键使用。 | 缺最终图标、物品 DataAsset 表和完整拖拽到场景放置链路。 |
| 合成 | 建造消耗已存在。 | 缺独立配方系统、合成 UI、产物进入背包/放置流程。 |
| 事件 | 有 7 天/21 事件推进、昼夜阶段、随机种子和跨天资源刷新。 | 事件选择 UI、钓鱼/上岛/完整潜水结果结算仍待做。 |
| 放置 | 已有建造网格、Deck 1x1。 | 缺可视化预览、可放置区域提示、更多模块类型。 |
| 船体 | 有浮动平台和逻辑网格。 | 缺“漂流进度/方向选择”；真实开船暂不建议。 |
| 终点 | 有事件数胜利条件雏形。 | 需要更明确的海岸/字幕/通关 UI 占位。 |
...
- 钓鱼、完整潜水、上岛可以出现在事件菜单里，但作为“待做/灰色项”；当前已实现的 X 下潜/浮上只覆盖移动和摄像机，不覆盖氧气/收益/事件结算。
