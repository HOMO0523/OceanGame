...
### `fix: add day snapshots and dive camera toggle` — 本次存档/潜水边界提交

- 做了什么：暂停菜单存档槽改为“下一次跨天快照槽”，Night→Morning 时自动写盘；读档强制回平台安全点并恢复摄像机；X 潜水让角色和摄像机一起下潜，再按 X 浮上并还原摄像机。
- 证明了什么：存档不再是任意时刻位置快照，而是符合“按天结算”的生存循环；潜水也不再只是数值改 Z，而有可验证的相机往返和海底移动状态。
- 答辩价值：可展示需求变更后如何冻结语义、先写红测、再用 UE 冷编译与自动化测试证明核心交互落地。
