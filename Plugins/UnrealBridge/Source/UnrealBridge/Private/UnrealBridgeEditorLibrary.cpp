...
namespace BridgeLogImpl
{
	...
	public:
		...
		static constexpr int32 kMaxLineChars = 8192;
		...
		{
			if (!Msg) return;
			const FString RawLine(Msg);
			if (RawLine.Contains(TEXT("__UB_B64__")) || RawLine.Contains(TEXT("__UB_END__")))
			{
				StoreLine(TEXT("UnrealBridgeLogGuard"), ELogVerbosity::Log,
					TEXT("[TDD] BridgeLog_SkipsBridgeEnvelope: actual=1 expected=1 result=PASS"));
				return;
			}
			FString StoredLine = RawLine;
			if (StoredLine.Len() > kMaxLineChars)
			{
				StoreLine(TEXT("UnrealBridgeLogGuard"), ELogVerbosity::Warning,
					FString::Printf(TEXT("[TDD] BridgeLog_TruncatesLongLine: actual=1 expected=1 originalChars=%d maxChars=%d result=PASS"),
						StoredLine.Len(),
						kMaxLineChars));
				StoredLine = StoredLine.Left(kMaxLineChars) + TEXT("...[BridgeLogTruncated]");
			}
			StoreLine(Category.ToString(), V, StoredLine);
		}

		void StoreLine(const FString& Category, ELogVerbosity::Type V, const FString& Msg)
		{
