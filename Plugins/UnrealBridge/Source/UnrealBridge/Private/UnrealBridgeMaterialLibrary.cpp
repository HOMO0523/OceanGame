...
FBridgeMaterialInfo UUnrealBridgeMaterialLibrary::GetMaterialInfo(const FString& MaterialPath)
{
...

	{
		...
		for (int32 i = 0; i < ParamInfos.Num(); ++i)
		{
			...
			bool Val = false;
			FGuid ExprGuid;
			MatInterface->GetStaticSwitchParameterValue(FHashedMaterialParameterInfo(ParamInfos[i]), Val, ExprGuid);
...
namespace BridgeMaterialImpl
{
	...
	static UMaterialExpression* FindExpressionByGuid(UMaterialFunction* Function, const FGuid& Guid)
	{
		if (!Function || !Guid.IsValid())
		{
			return nullptr;
		}
		for (const TObjectPtr<UMaterialExpression>& ExprPtr : Function->GetExpressions())
		{
			UMaterialExpression* Expr = ExprPtr.Get();
			if (Expr && Expr->MaterialExpressionGuid == Guid)
			{
	...
	/** Find the index of an output pin by name on an expression, or INDEX_NONE. */
	static int32 FindOutputIndexByName(UMaterialExpression* Expr, const FString& Name)
	{
		if (!Expr)
		{
			return INDEX_NONE;
		...
		{
			const FString GotStr = NormalizePinName(Outputs[Index].OutputName.ToString());
			if (GotStr.Equals(Target, ESearchCase::IgnoreCase))
			{
				return Index;
			}
		}
		if (Target.IsEmpty() && Outputs.Num() == 1)
		{
			return 0;
		}
		return INDEX_NONE;
	...
	FGuid DstGuid, const FString& DstInputName)
{
	...
	{
		Material->MarkPackageDirty();
		UE_LOG(LogTemp, Log, TEXT("[TDD] BridgeMaterial_ConnectFallback: actual=0 expected=0 material=%s"),
			*MaterialPath);
		return true;
	}

	const int32 SrcOutputIndex = FindOutputIndexByName(Src, SrcOutputName);
	const int32 DstInputIndex = FindInputIndexByName(Dst, DstInputName);
	const bool bFallbackOK = SrcOutputIndex != INDEX_NONE && DstInputIndex != INDEX_NONE;
	UE_LOG(LogTemp, Log, TEXT("[TDD] BridgeMaterial_ConnectFallback: actual=%d expected=1 material=%s src_out=%s dst_in=%s"),
		bFallbackOK ? 1 : 0, *MaterialPath, *SrcOutputName, *DstInputName);
	if (!bFallbackOK)
	{
		return false;
	}

	FExpressionInput* Input = Dst->GetInput(DstInputIndex);
	if (!Input)
	{
		return false;
	}
	...
	Input->OutputIndex = SrcOutputIndex;
	Material->MarkPackageDirty();
	return true;
}
	...
	const FString& Value)
{
	...
	if (!Material)
	{
		UMaterialFunction* Function = LoadObject<UMaterialFunction>(nullptr, *MaterialPath);
		if (!Function) return false;

		UMaterialExpression* Expr = FindExpressionByGuid(Function, Guid);
		if (!Expr) return false;
		...
		if (bOK)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("[TDD] BridgeMaterial_SetFunctionExpressionProperty: actual=1 expected=1 result=PASS asset=%s property=%s"),
				*MaterialPath,
				*PropertyName);
		}
		return bOK;
	...
	const TArray<FBridgeExpressionPropSet>& Properties)
{
	...
	if (!Material)
	{
		...
		{
			Function->MarkPackageDirty();
			UE_LOG(LogTemp, Log, TEXT("[TDD] BridgeMaterial_SetFunctionExpressionProperty: actual=1 expected=1 result=PASS asset=%s applied=%d"),
				*MaterialPath,
				Applied);
		}
		return Applied;
...
namespace BridgeMaterialImpl
{
		...
		FString& OutError)
	{
		...
		if (TypeLower == TEXT("staticswitch") || TypeLower == TEXT("switch") || TypeLower == TEXT("bool"))
		{
			...
			FGuid ActualGuid;
			MIC->GetStaticSwitchParameterValue(Info, ActualBool, ActualGuid);
			UE_LOG(LogTemp, Log, TEXT("[TDD] BridgeMaterial_StaticSwitchApplied: actual=%d expected=%d param=%s"),
				ActualBool ? 1 : 0, V ? 1 : 0, *P.Name);
			return ActualBool == V;
		}
